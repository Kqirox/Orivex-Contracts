# Orivex Webhooks Documentation

Webhooks allow your application to receive real-time notifications about events that happen in the Orivex platform. Instead of polling our API for updates, we'll send HTTP requests to your endpoint when specific events occur.

## Overview

Webhooks are a powerful way to integrate with Orivex and build responsive applications. For example, you could:
...
app.post('/webhooks/orivex', (req, res) => {
  const event = req.body;

  // Verify signature
  const signature = req.headers['x-orivex-signature'];
...
1. Log in to the Orivex Partner Dashboard
...
{
  "url": "https://yourapp.com/webhooks/orivex",
...
All webhook requests include a signature header that you must verify to ensure the request came from Orivex.

**Header:** `X-Orivex-Signature`
...
# Usage in Flask
@app.route('/webhooks/orivex', methods=['POST'])
def webhook():
    signature = request.headers.get('X-Orivex-Signature')
...
For additional security, you can whitelist Orivex's IP addresses:
...
```javascript
app.post('/webhooks/orivex', async (req, res) => {
...
async function sendWelcomeEmail(email, moduleTitle, score) {
  const msg = {
    ...
    html: `
      ...
      <a href="https://orivex.io/learn">Continue Learning →</a>
    `,
  };
...
- **Documentation**: [https://docs.orivex.io/webhooks](https://docs.orivex.io/webhooks)
- **Discord**: #webhooks channel
- **Email**: orivex@toneflix.net
- **Status Page**: [https://status.orivex.io](https://status.orivex.io)
